...
import { resolveProgramRecruiterAlt } from "@/alt-text/programs/soet";
...
const RecruiterCarousel = ({ logos }: RecruiterCarouselProps) => {
    ...
    if (!mounted) {
        return (
            <div className="w-full min-h-[80px] sm:min-h-[96px] md:min-h-[112px]" />
        );
    }
                ...
                {logos.map((logo) => (
                    ...
                    >
                        <Image
                            ...
                            width={120}
                            height={120}
                            alt={resolveProgramRecruiterAlt(
                                logo?.url,
                                logo?.name,
            ...
            <div className="flex animate-marquee whitespace-nowrap gap-3 sm:gap-4 py-2">
                {logos
                    ...
                    .map((logo, index) => (
                        <div
                            key={`${logo.id}-${index}`}
                            className="w-36 h-20 sm:w-32 sm:h-20 md:w-36 md:h-22 flex items-center justify-center bg-white rounded-[2px] md:rounded-[4px] p-2 flex-shrink-0 transition-transform duration-300 hover:scale-105 customCarousel"
                        >
                            <Image
                                src={`${STRAPI_URL}${logo?.url}`}
                                width={120}

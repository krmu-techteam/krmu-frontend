...
const UniversityComparisonTable = ({
    ...
    return (
        <div className="w-full border border-[#00559B] rounded-[15px] py-4 md:py-4 xl:py-7">
            ...
            <div className="hidden lg:grid grid-cols-3 gap-6 items-center pb-5 border-b border-[#00559B] px-6 lg:px-8 xl:px-14 lg:mb-6">
            ...
            <div className="flex flex-col">
                {list1?.map((item, index) => {
                    ...
                    return (
                        <div
                            key={item?.id || index}
                            className="py-4 px-4 sm:px-6 lg:px-8 xl:px-14"
                        >
                            {/* ── Mobile & Tablet View (< lg) ── */}
                            <div className="lg:hidden flex flex-col gap-3">
                                {/* Feature Title */}
                                <div className="text-[16px] sm:text-[17px] text-white font-semibold flex items-center gap-2">
                                    <span className="w-1.5 h-1.5 rounded-full bg-[#00559B] inline-block shrink-0" />
                                    {item?.listtext}
                                </div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mt-1">
                                    ...
                                    <div className="bg-[#facc15]/10 border border-[#facc15]/25 rounded-[4px] p-3 sm:p-4 flex flex-col gap-1.5">
                                        <div className="text-[#facc15] text-xs font-semibold uppercase tracking-wider flex items-center gap-1.5">
                                            <Check
                                                className="w-4 h-4 text-[#facc15] shrink-0"
                                                strokeWidth={2.5}
                                            />
                                            <span>
                                                {list3?.listheading || "SOET"}
                            ...
                            <div className="hidden lg:grid grid-cols-3 gap-6 items-start">
                                {/* Feature Title */}
                                <div className="text-[16px] text-white/90 font-medium md:pt-1">
                                    {item?.listtext}
                                </div>
                                ...
                                <div className="flex items-start gap-4">
                                    <X
                                        ...
                                        strokeWidth={2.5}
                                    />
                                    <span className="text-[15px] md:text-[16px] text-[#ff9999]">
                                        {tradItem?.listtext}
                                    </span>
                                </div>
                                ...
                                <div className="flex items-start gap-4">
                                    <Check
                                        ...
                                        strokeWidth={2.5}
                                    />
                                    <span className="text-[15px] md:text-[16px] text-[#fef08a]">
                                        {krmuItem?.listtext}
                                    </span>
                                </div>
                            </div>

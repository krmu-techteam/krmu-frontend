...
const HallofFame: React.FC<HALLFAMEPROP> = ({ fameData }) => {
  ...
  return (
    <div className="flex flex-col w-full">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 px-0 md:px-0">
        {displayedData &&
          displayedData.map((fame) => {
            return (
              <div
                key={fame?.id}
                className="relative p-[2px] rounded-[4px] min-h-[305px] h-full text-left transition-transform duration-300 hover:-translate-y-1"
                style={{
                  background:
                    "linear-gradient(136.56deg, rgba(0, 102, 255, 0.5) 3.59%, rgba(255, 255, 255, 0.5) 97.91%)",
                }}
              >
                <div className="bg-[#061623] w-full h-full rounded-[2px] p-6 md:p-8 flex flex-col backdrop-blur-sm">

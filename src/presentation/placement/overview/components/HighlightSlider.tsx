...
const getSuccessStoryAlt = (src: string, index: number) => {
    const filename =
        src
            .split("/")
            .pop()
    ...
    return cleanName
        ? `${cleanName} – K.R. Mangalam University student placement success story`
        : `K.R. Mangalam University student placement success story ${index + 1}`;
};
                    ...
                    {SUCCESS_STORIES.map((story, i) => (
                        <div key={i} className="relative group">
                            <div className="relative w-full aspect-square max-h-[460px]">
                                <Image
                                    src={story.image}
                                    alt={getSuccessStoryAlt(story.image, i)}
                                    fill

...
export async function getSingleNewsAndEventsWP(newsandeventsslug: string = "") {
  const res = await fetch(
    `${KRMUWordUrl}/wp-json/wp/v2/events-and-news?slug=${newsandeventsslug}&_fields=id,title,excerpt,content,featured_media,acf.event_images,acf.keywords,slug,yoast_head_json`,
    {

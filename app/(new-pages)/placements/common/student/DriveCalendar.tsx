  ...
  useEffect(() => {
    const fetchDrives = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error("Failed to fetch drive calendar");
        }
              ...
              records.map((record) => (
                ...
                >
                  ...
                  <div className="min-w-0 font-poppins text-[10px] leading-[1.6] text-[#333] lg:text-xs">
                    <p>Float Date: {formatDate(record.float_date)}</p>

                    <p>Drive Date: {formatDate(record.drive_date)}</p>
                  </div>
        ...
        {!error && loading && (
          <div className="md:hidden">
            {Array.from({
              ...
              <article key={index} className="border-b border-[#d8d5d0] py-6">

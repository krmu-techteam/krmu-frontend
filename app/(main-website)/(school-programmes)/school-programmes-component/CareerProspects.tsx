            ...
            <div className="w-full xl:w-1/2 flex flex-col">
              ...
              <div className="flex-1">
                {careercards?.map((card) => (
                  <div key={card?.id} className="flex gap-5 mb-8">
                    <div className="flex-shrink-0">
                      <span className="flex items-center justify-center w-12 h-10 rounded-md text-white bg-[#0a41a1] text-lg font-semibold shadow-sm">
                        {card?.num}
                      </span>
                    </div>
                    <div className="flex-1">
                      <h4 className="font-semibold text-2xl mb-1 text-gray-900">{card?.title}</h4>
                      <p className="leading-relaxed text-gray-600 text-base md:text-md">{card?.description}</p>
                    </div>
                  </div>
                ))}
              </div>
